...
class DigitCmdVelSubscriber(Node):
    # Constructor
    def __init__(self,node_name,topic_name):
        ...
        if sys.argv[1] == 'sim':
            ...
            #  self.ws = BasicClient('ws://10.10.1.150:8080', protocols=['json-v1-agility'])
            self.ws.daemon = False
        else:
    ...
    def cmd_vel_callback(self,msg):
        # Parse message
        yaw_vel = msg.angular.z # body frame
        x_vel = msg.linear.x    # body
        y_vel = msg.linear.y    # body
        ...
        "action-move",
            {
            "velocity": {"rpyxy": [0,0,yaw_vel,x_vel,y_vel]},
            "mobility-parameters": {"avoid-obstacles":False}
            }
        ]
        self.ws.send(json.dumps(json_msg))
        self.get_logger().info('Sent action-move command')
...
def main(args=None):
    ...
    node_name = 'sub_cmd_vel'
    topic_name = 'autonomous_cmd_vel'
    node_cmd_vel = DigitCmdVelSubscriber(node_name, topic_name)
